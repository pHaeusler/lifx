...
class PacketType:
    ...
    PAN_GATEWAY = PacketTypeDefFromBulb('PAN Gateway', 0x03, 'service port', '<bI')
    ...
    LIGHT_STATUS = PacketTypeDefFromBulb('Light Status', 0x6b, 'hue saturation brightness kelvin dim power label tags', '<HHHHhH32sQ')

    # Time
    GET_TIME = PacketTypeDefToBulb('Get Time', 0x04, None, None)
    SET_TIME = PacketTypeDefToBulb('Set Time', 0x05, 'time', '<Q')
    TIME_STATE = PacketTypeDefFromBulb('Time State', 0x06, 'time', '<Q')
    ...
    MESH_INFO = PacketTypeDefFromBulb('Mesh Info', 0x0d, 'signal tx rx mcu_temperature', '<fiih')
...
class Packet:
    ...
    @classmethod
    def ToBulb(cls, packet_type, target, site, ack, *payload_data):
        payload_bytes = b""
        ...
        header_data = (cls.header_size + len(payload_bytes), cls.protocol, 0, target_vs, 0, site_vs, ack, 0, packet_type.code, 0)
